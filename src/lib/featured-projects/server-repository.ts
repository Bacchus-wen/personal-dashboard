...
import {
  FEATURED_PROJECT_COLUMNS,
  createFeaturedProjectRepository,
  type FeaturedProjectDatabaseClient,
  type FeaturedProjectDatabaseFilter,
  type FeaturedProjectDatabaseRow,
} from "./repository";
...
function createSupabaseFeaturedProjectDatabaseClient(
  client: SupabaseClient,
): FeaturedProjectDatabaseClient {
  return {
    async selectMany(request) {
      ...
      if (request.search) {
        ...
        query = query.or(
          `name.ilike.%${escaped}%,summary.ilike.%${escaped}%,recommendation.ilike.%${escaped}%`,
        );
      }
      ...
      return { rows: (data ?? []) as FeaturedProjectDatabaseRow[] };
    },
    ...
    async selectOne(request) {
      ...
      if (error) throw error;
      return { row: (data as FeaturedProjectDatabaseRow | null) ?? null };
    },

    async insert(request) {
      const { data, error } = await from(client, request.table)
        .insert(request.values ?? {})
        .select(request.columns ?? FEATURED_PROJECT_COLUMNS)
        .single();
      if (error) throw error;
      return { row: data as FeaturedProjectDatabaseRow };
    },
...
export function getFeaturedProjectRepository() {
  return createFeaturedProjectRepository(
    createSupabaseFeaturedProjectDatabaseClient(createSupabaseAdminClient()),
  );
}
