...
export default async function AboutPage() {
  ...
  return (
    <PageShell
      ...
      title={configuration.settings.displayName.toUpperCase()}
      ...
      <section className="about-editorial">
        <aside className="about-identity glass">
          <div className="about-avatar-mark">
            {configuration.settings.displayName.slice(0, 1).toUpperCase()}
          </div>
          <p className="eyebrow">PROFILE NOTE</p>
          <h2>{configuration.settings.displayName}</h2>
          <p className="status">{configuration.settings.statusText}</p>
          <p className="about-signature">Build slowly, notice carefully.</p>
        </aside>

        <article className="about-card glass">
          <h2>
            <span className="hash">#</span> Hi! I&apos;m{" "}
