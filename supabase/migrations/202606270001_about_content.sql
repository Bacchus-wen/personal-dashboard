-- Editable About page body (Markdown). Stored on the singleton site_settings
-- row. Empty by default; the application falls back to its built-in default
-- About content when this is blank.

alter table public.site_settings
  add column if not exists about_body text not null default '';

-- Recreate the publisher to persist about_body alongside the other settings.
...
begin
  ...
  values (
    ...
    coalesce(settings->'navigation_visibility', '{
      "plans": true,
      "articles": false,
      "works": true,
      "about": true,
